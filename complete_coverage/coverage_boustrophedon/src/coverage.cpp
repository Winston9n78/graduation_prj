...
Coverage::Coverage()
{
  ...
  // Set up partition
  m_partition.initialize(nh, m_x0, m_y0, m_x1, m_y1, m_tileResolution, m_scanRange); //建了一个网格图

  // Set up subscribers
  ros::Subscriber sub = nh.subscribe("inflated_map", 1000, &Coverage::mapCallback, this); //更新地图，我的版本应该没有这个功能

  // Set up publishers
  m_goalPub = nh.advertise<geometry_msgs::PoseStamped>("move_base_simple/goal", 1000); //把这些发布的替换成我固定的
  ...
  mainLoop(nh); //修改这里，改成发布全部目标路径？就运行一下publishGoal，不需要网格，坐标的获取在updatePose函数中
}
...
void Coverage::mainLoop(ros::NodeHandle nh)
{
  ...
  while (nh.ok())
  {
    ...
    if (m_mapInitialized)
    {
      ...
      int gx, gy;
      m_partition.worldToGrid(m_pose.x, m_pose.y, gx, gy); //将原来坐标转换为在网格中的坐标
      ...
      Goal goal = updateWPs(gx, gy);//通过这个自身坐标获得目标坐标，获得一个下一个goal的坐标
...
Coverage::Goal Coverage::updateWPs(int gx, int gy)
{
      ...
      m_partition.hasCompleteCoverage() && !m_finished)
  {
    ...
    auto path = aStarSPT(m_partition, {gx, gy}, {startX, startY}); //通过astar算法形成了path,path再赋值给m_waypoint(),由目标点，和自身坐标形成
    for (auto it = path.begin(); it != path.end(); it++)
    {
      m_waypoints.push_back(*it);
    }
...
bool Coverage::checkDirection(Direction dir, int gx, int gy) //在这里加goal
{
...
void Coverage::publishGoal(int gx, int gy, Goal goal) //
{
  ...
  double x, y;
  m_partition.gridToWorld(goal.gx, goal.gy, x, y); //通过这里将期望坐标转换为网格坐标里的坐标
  // double yaw;
