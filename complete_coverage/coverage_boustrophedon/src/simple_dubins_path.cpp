...
SimpleDubinsPath::SimpleDubinsPath()
{
  ...
  //                                         &SimpleDubinsPath::onInput, this); //由coverage.cpp发布

  // ros::Subscriber sub = nh.subscribe("simple_dubins_path/goal", 1000,
  //                                    &SimpleDubinsPath::onGoal, this);

  m_pointPub = nh.advertise<geometry_msgs::PoseStamped>("simple_dubins_path", 1000); //在onIput里面把路径很严格计算才发布出来

  //固定路径，三个点
  nav_msgs::Path path;
  geometry_msgs::PoseStamped point0;
  ...
  while (ros::ok())
  {
    ...
    point0.pose.position.x = 3;
    point0.pose.position.y = 6;

    std::cout << "发布的目标点坐标:(" << point0.pose.position.x << ", " << point0.pose.position.y << ")" << std::endl;

    m_pointPub.publish(point0);
...
void SimpleDubinsPath::onGoal(const geometry_msgs::PoseStamped& goal)
{
  ...
  if (counter == 1)
  {
    ROS_INFO("Received start pose"); //收到的这个从哪里发过来的呢
    start = goal;
  }
  else if (counter == 2)
  {
    ...
    nav_msgs::Path path;//我的发布里面要实现这三句话
...
void SimpleDubinsPath::onInput(const coverage_boustrophedon::DubinInput& input)
{
  ...
  m_pathPub.publish(path); //读取path并发布给guidance
}
