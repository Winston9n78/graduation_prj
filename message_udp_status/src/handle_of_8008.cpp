...
int recv_num_8008;
int send_num_8008;
char send_buf_8008[50] = "";
char recv_buf_8008[1024] = "";
...
void recieve_thread_function_8008(){
  ...
  while(1){
    recv_num_8008 = recvfrom(sock_fd_8008, recv_buf_8008, sizeof(recv_buf_8008), 0, (struct sockaddr *)&addr_client_8008, (socklen_t *)&len);
    if(recv_num_8008 < 0)
    {
      perror("recvfrom error:");
      exit(1);
    }
    recv_buf_8008[recv_num_8008] = '\0';
    ...
    if(recv_num_8008 > 0)
    {
        ...
        {
          map_path.data.push_back(float(std::stod(vStr[i])));
        }
    }
  }
}
